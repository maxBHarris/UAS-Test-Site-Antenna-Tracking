...
function tilt_gimbal(angle)
    ...
    local pwm_output = pwm_range*(normalize(angle, TILT_MIN_ANGLE, TILT_MAX_ANGLE))+TILT_MIN_PWM
...
function pan_gimbal(angle)
    ...
    local pwm_output = pwm_range*(normalize(angle, PAN_MIN_ANGLE, PAN_MAX_ANGLE))+PAN_MIN_PWM
...
function calculate_pan_and_tilt(roll, pitch, yaw, target_lat, target_lng, target_alt, curr_pos)
    ...
    pan_gimbal(-1*(pan_aim_angle + math.deg(yaw) + PAN_OFFSET))
    tilt_gimbal(-1*(tilt_aim_angle))
end
